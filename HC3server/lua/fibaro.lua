...
function net.HTTPCall()
    return {
        request = function(_, url, opts)
            opts = opts or {}
            return __HTTP(url, opts.options, opts.success, opts.error)
        end
    }
end
...
api = {}
local function res(val,code)
    if type(val)=='string' then return json.decode(val),code
    else return nil, code end
end

function api.get(url)
    return res(___pyhooks.api_get(url))
end
function api.post(url, data)
    return res(___pyhooks.api_post(url, json.encode(data)))
end
function api.put(url, data)
    return res(___pyhooks.api_put(url, json.encode(data)))
end
function api.delete(url, data)
    return res(___pyhooks.api_delete(url, json.encode(data)))
end

function __fibaro_get_global_variable(name)
    return res(___pyhooks.fibaro_get_global_variable(name))
end

function __fibaro_get_device(id)
    return res(___pyhooks.fibaro_get_device(id))
end

function __fibaro_get_devices()
    return res(___pyhooks.fibaro_get_devices())
end

function __fibaro_get_room(id)
    res(___pyhooks.fibaro_get_room(id))
end

function __fibaro_get_scene(id)
    return res(___pyhooks.fibaro_get_scene(id))
end

function __fibaro_get_device_property(id, prop)
    return res(___pyhooks.fibaro_get_device_property(id,prop))
end

function __fibaro_get_breached_partitions()
    return res(___pyhooks.fibaro_get_breached_partitions(id,prop))
end

function __fibaro_add_debug_message(tag, str, typ)
    assert(str, "Missing tag for debug")
    return res(___pyhooks.fibaro_add_debug_message(tag, str, typ))
end
